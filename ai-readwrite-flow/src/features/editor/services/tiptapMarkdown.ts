...
const BLOCK_TYPES = new Set(['paragraph', 'heading', 'codeBlock', 'bulletList', 'orderedList', 'listItem', 'blockquote'])
...
const getNodeType = (node: TipTapNode): string | null =>
  typeof node.type === 'string' ? node.type : null

const getChildren = (node: TipTapNode): unknown[] =>
...
const getMarkType = (mark: TipTapMark): string | null =>
  typeof mark.type === 'string' ? mark.type : null
