...
| 18-QA-010 | Markdown preview | In Writer, type markdown in Content (e.g., `## Title` + `- item` + `**bold**`); click Preview; then back to Edit | Preview renders markdown read-only; switching back preserves text exactly | Pending | Fixed converter bug where lines starting with `**` were mis-normalized as list items (`* *bold**`), causing “bold” to appear as a bullet. |
