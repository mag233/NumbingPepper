...
| 18-QA-033 | Writer global search | In Writer, click Search; query a term that appears in Content/Context/References/Chat/Studio; toggle filters and click results | Modal returns scoped matches; clicking a result scrolls/focuses the correct area; filters update counts | Partial (2026-01-07) | **Pass:** Filter chips now retain the correct grey counts even when toggled off; Content/Context jump with highlight. **Issues:** (1) Clicking Chat results closes the modal and no highlight is visible, so it is hard to confirm the jump; keep the modal open and ensure the `search-highlight-flash` class applies to `writer-chat-*` nodes in [WriterSearchModal.tsx](ai-readwrite-flow/src/features/editor/components/WriterSearchModal.tsx#L205-L245). (2) References behave the same—modal closes but the list neither scrolls nor highlights, so users think nothing happened; add a highlight class on `writer-reference-*` elements and avoid forcing modal close. (3) Studio scope is no longer required; remove Studio from filters/results to simplify the UI. |
...
| 18-QA-019 | Writer outline | In Writer Content, add headings like `# Title`, `## Section`; verify Outline appears in left sidebar (shows H-level + indentation); click an item jumps/scrolls editor near that heading (in Edit mode) | Outline matches headings; click-to-jump works | Partial (2026-01-07) | **Pass:** Edit mode jump path still works. **Issues:** (1) Preview mode now hides the outline rows entirely (only the “Switch to Edit mode to navigate” notice renders even though three headings are detected); headings must remain visible but non-interactive. (2) When Preview is active, clicking an outline row should show a disabled affordance (e.g., grey text + tooltip) instead of doing nothing. |
